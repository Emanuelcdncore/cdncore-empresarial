...
const TextType = ({ text, as: Component = 'div', typingSpeed = 50, initialDelay = 0, pauseDuration = 2000, deletingSpeed = 30, loop = true, className = '', showCursor = true, hideCursorWhileTyping = false, cursorCharacter = '|', cursorClassName = '', cursorBlinkDuration = 0.5, textColors = [], variableSpeed, onSentenceComplete, startOnVisible = false, reverseMode = false, ...props }) => {
  ...
  useEffect(() => {
    ...
    const execute = () => {
      if (isDeleting) {
        ...
        } else { timeout = setTimeout(() => setDisplayedText(prev => prev.slice(0, -1)), deletingSpeed); }
      } else {
    ...
    if (currentCharIndex === 0 && !isDeleting && displayedText === '') { timeout = setTimeout(execute, initialDelay); } else { execute(); }
  ...
  return createElement(Component, { ref: containerRef, className: `text-type ${className}`, ...props },
    createElement('span', { className: 'text-type__content', style: { color: textColors.length ? textColors[currentTextIndex % textColors.length] : 'inherit' } }, displayedText),
